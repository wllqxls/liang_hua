...
- 成交量吸收反转研究必须复用 `src/research/event_factors.py` 与 `scripts/research_event_factors.py`，不得创建另一套研究台。事件 A 固定为成交量相对前 20 根均量 >= `3.0`、真实波幅/ATR(14) <= `0.8`、过去 3 根收盘位移绝对值 >= `1.0 ATR`；研究方向与位移相反。事件 B 固定为随后 3 根内达到 `0.5 ATR` 的反向移动。先只跑 5m 的 BTC/ETH 2024/2025；仅当四个切片成本后 1h 平均收益全部为正时才允许继续 15m。
