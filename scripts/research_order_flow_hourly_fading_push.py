"""Run the frozen hourly fading-push reversal study."""
...
from src.research.order_flow_hourly_fading_push import (
    HourlyFadingPushResearchSlice,
    primary_validation_passed,
    run_hourly_fading_push_research,
)


DEFAULT_DATA_ROOT = PROJECT_ROOT / 'data' / 'order_flow' / 'binance_um'
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / 'results' / 'research' / 'order_flow'
DEFAULT_REPORT = PROJECT_ROOT / 'docs' / 'research' / 'order-flow-hourly-fading-push-report.md'
logger = logging.getLogger(__name__)


def write_report(slices: Sequence[HourlyFadingPushResearchSlice], output_path: Path) -> None:
    """Write the reproducible frozen-study report."""
    ...
    for item in slices:
        lines.append(f'| {item.symbol} | {item.year} | {item.events} | {item.eligible_rows} | {item.excluded_metric_rows} | `{item.dataset_path.relative_to(PROJECT_ROOT)}` |')
    for item in slices:
...
def _slice_lines(item: HourlyFadingPushResearchSlice) -> list[str]:
    lines = [
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run frozen hourly fading-push reversal research.')
...
def main() -> None:
    ...
    args = _parse_args()
    slices = run_hourly_fading_push_research(data_root=args.data_root, output_root=args.output_root)
