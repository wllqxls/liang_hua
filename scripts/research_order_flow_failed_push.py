"""Run the frozen failed-push order-flow reversal study."""
...
from src.research.order_flow_failed_push import (
    FailedPushResearchSlice,
    primary_validation_passed,
    run_failed_push_reversal_research,
)

DEFAULT_DATA_ROOT = PROJECT_ROOT / 'data' / 'order_flow' / 'binance_um'
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / 'results' / 'research' / 'order_flow'
DEFAULT_REPORT = PROJECT_ROOT / 'docs' / 'research' / 'order-flow-failed-push-reversal-report.md'
logger = logging.getLogger(__name__)


def write_report(slices: Sequence[FailedPushResearchSlice], output_path: Path) -> None:
    """Write the frozen-study report and its prespecified validation verdict."""
...
def _slice_lines(item: FailedPushResearchSlice) -> list[str]:
    lines = [
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run frozen failed-push reversal research.')
...
def main() -> None:
    ...
    args = _parse_args()
    slices = run_failed_push_reversal_research(
